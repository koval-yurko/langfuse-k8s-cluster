resource "helm_release" "langfuse" {
  ...
  values = [templatefile("${path.module}/values.yaml.tpl", {
    rds_host        = split(":", data.tfe_outputs.deps.nonsensitive_values.rds_endpoint)[0]
    rds_password    = data.tfe_outputs.deps.values.rds_password
    s3_bucket       = data.tfe_outputs.deps.nonsensitive_values.s3_bucket_name
    s3_region       = data.tfe_outputs.deps.nonsensitive_values.s3_bucket_region
    irsa_role_arn   = data.tfe_outputs.deps.nonsensitive_values.irsa_role_arn
