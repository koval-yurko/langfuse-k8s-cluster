...
resource "aws_vpc_security_group_ingress_rule" "rds_from_eks" {
  ...
  referenced_security_group_id = data.tfe_outputs.network.nonsensitive_values.node_security_group_id
  from_port                    = 5432
  to_port                      = 5432
  ip_protocol                  = "tcp"
}
...
module "rds" {
  ...
  subnet_ids             = data.tfe_outputs.network.nonsensitive_values.public_subnet_ids
